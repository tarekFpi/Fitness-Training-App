...
class DateConverter {
  ...
  ///    Sat, 09, September,2024 date formate
  ...
  static  String formattedDate2(String inputDate) {

    //String inputDate = "2024-12-02T00:00:00.000Z";
    DateTime dateTime = DateTime.parse(inputDate);

    // Format the date
    String formattedDate = DateFormat('EEE, dd, MMMM, yyyy').format(dateTime);
 // Output: Mon, 02, December, 2024
    return "$formattedDate";
  }



  static  String convertTimeToMinutes(String time) {
 ...
 // date time formater ======================
  ...
  static String dateTimeFormater(String dateString){

    DateTime dateTime = DateTime.parse(dateString);

    String formattedDate1 = DateFormat('yyyy-MM-dd').format(dateTime);
...
static List<String> calculateAgeAndLifeSpan(String dob, int targetAge) {
  ...
  if (days < 0) {
    months--;
    days += DateTime(currentDate.year, currentDate.month, 0).day; // Add days of previous month
  }
  if (months < 0) {
    years--;
    months += 12;
...
static List<String> calculateRemainingLife(String dob, int targetAge) {
  ...
  DateTime targetDate = DateTime(birthDate.year + targetAge, birthDate.month, birthDate.day);
  ...
  if (remainingDays < 0) {
    remainingMonths--;
    remainingDays += DateTime(currentDate.year, currentDate.month + 1, 0).day; // Adjust for negative days
  }
  ...
  int targetAgeDays = targetAge * 365; // Approximation of target age in days
  double lifeRemainingPercent = ((targetAgeDays - totalDaysLived) / targetAgeDays) * 100;
